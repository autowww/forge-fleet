"""Generic Docker Compose operations for Fleet-managed service records."""
...
def compose_argv(root: Path, rel_files: list[str]) -> list[str]:
    argv = ["docker", "compose"]
    for f in rel_files:
...
def compose_ps(root: Path, rel_files: list[str]) -> tuple[list[dict[str, Any]], str | None]:
    ...
    cmd = compose_argv(root, rel_files) + ["ps", "-a", "--format", "json"]
    try:
        r = subprocess.run(
            cmd,
            cwd=str(root),
...
def market_app_host_port_from_compose_ps(rows: list[dict[str, Any]]) -> dict[str, Any] | None:
    """
    Parse ``docker compose ps --format json`` rows for ``market-app`` published ``9792`` port.

    Typical ``Ports`` string: ``0.0.0.0:19792->9792/tcp``.
    """
    pat = re.compile(
        r"(?:^|[\s,])(?:0\.0\.0\.0|\:\:|\:\:\:|127\.0\.0\.1)\:(\d+)->9792/(?:tcp|udp)",
        re.I,
    )
    for row in rows:
        ...
        if "market-app" not in name and "market_app" not in name:
            continue
        ...
        try:
            ...
            continue
        return {"host_port": hp, "container_port": 9792, "ports_preview": ports[:240]}
    return None
...
def start_for_record(record: dict[str, Any]) -> dict[str, Any]:
    ...
    extras = [str(x) for x in raw_cf] if isinstance(raw_cf, list) else []
    rel = resolve_compose_files(root, extras)
    cmd = compose_argv(root, rel) + ["up", "-d"]
    with _LOCK:
        try:
            r = subprocess.run(
                cmd,
...
def stop_for_record(record: dict[str, Any]) -> dict[str, Any]:
    ...
    extras = [str(x) for x in raw_cf] if isinstance(raw_cf, list) else []
    rel = resolve_compose_files(root, extras)
    cmd = compose_argv(root, rel) + ["down"]
    with _LOCK:
        try:
            r = subprocess.run(
                cmd,
