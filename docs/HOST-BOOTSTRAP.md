...
If you cannot install buildx, set **`FLEET_DOCKER_BUILDKIT=0`** in the Fleet environment file (see [`systemd/environment.example`](../systemd/environment.example)) so template builds use the legacy Docker builder. See [`docs/CONTAINER-TEMPLATES.md`](CONTAINER-TEMPLATES.md) for `FLEET_DOCKER_BUILDKIT` behavior.

## 3. User-level Fleet (systemd --user)

If you use [`./git-install.sh --user`](GIT-INSTALL.md) (see that doc), enable lingering so the service can start without an interactive login:

```bash
loginctl enable-linger "$USER"
```

See [`docs/GIT-INSTALL.md`](GIT-INSTALL.md) for ports (**18766** user vs **18765** system) and data directories.
...
Full clone and systemd narrative: **[docs/GIT-INSTALL.md](GIT-INSTALL.md)**.
