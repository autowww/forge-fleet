...
Canonical **Forge LCDL** handbook (Firebase Hosting project **`forge-sdlc-lcdl`**) is built from the **`forge-lcdl`** Markdown tree via **`forge-lcdl-website`**. Narrative onboarding: **`docs/WHAT-IS-LCDL.md`** in **`forge-lcdl`**. **Forge Fleet** handbook (project **`forge-sdlc-fleet`**) stays **`forge-fleet-website`**.
