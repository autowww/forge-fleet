...
| GET | `/v1/admin/snapshot` | **Read-only** JSON: `meta.integrations` includes **`forge_console_url`**, **`suggested_forge_llm_compose_root`**, **`container_layout`**, **`container_types_version`**, **`forge_llm_services`**, and **`orchestration`** (same shape as telemetry: running compose totals by type + running jobs by `container_class`). Plus **`meta.version`**, **`meta.energy_ledger_kwh`**, `host`, **`node`**, `jobs_by_status`, `jobs_recent`, `active_workers`. Throttled telemetry append stores `{ "host", "orchestration" }` (see `/v1/telemetry`). |
...
| GET | `/admin/` | Admin UI — **kitchensink** `forge-theme.css` + `forge-fleet-admin.css` + `forge-theme.js` (Light / Dark / System), Bootstrap 5; polls snapshot. **Overview**: KPI tiles (only when the host exposes that telemetry), **~1h** CPU/RAM chart (optional **linear 0–100%** Y via `localStorage` `forgeFleetChartLinearY`, stronger CPU smoothing vs RAM), **Refresh metrics**, workload strip (compose + job counts over time), container-type swimlanes. **Load** tile: **1m** dial + raw **L 1m·5m·15m** line (no duplicate bar rows); gauge eases like CPU with a short **ghost** needle fade. **Update Fleet** appears on the version line when git self-update is configured, GitHub `master` is ahead, and the install is **not** a system tree under `/opt/forge-fleet` (system installs: **View commits** only). Zone tints: **under 25% / 25–50 / 50–75 / over 75%**. |
