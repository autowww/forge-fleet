...
| GET | `/v1/cooldown-summary?period=…` | Aggregated **LLM thermal cooldown** wait time (seconds) for the SQLite **`fleet_cooldown_events`** table: same **`period`** values and aliases as **`/v1/telemetry`**. Response includes **`total_cooldown_s`**, **`event_count`**, **`window`**, **`store_bounds`**. |
