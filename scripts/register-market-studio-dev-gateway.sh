#!/usr/bin/env bash
# Register Fleet app gateway for DEV Market Studio (loopback :19793).
set -euo pipefail
...
SERVICE_ID="${FORGE_MARKET_DEV_GATEWAY_ID:-market-studio-dev}"
UPSTREAM="${FORGE_MARKET_DEV_GATEWAY_UPSTREAM:-http://127.0.0.1:${FORGE_MARKET_STUDIO_HOST_PORT:-19793}}"

log() { printf 'register-market-studio-dev-gateway: %s\n' "$*"; }
...
curl -fsS -X PUT \
  ...
  -d "{\"upstream\":\"${UPSTREAM}\",\"inject_bearer\":true,\"app_bearer_env\":\"FORGE_MARKET_STUDIO_API_TOKEN\"}" \
